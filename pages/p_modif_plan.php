...
if (!isset($_GET['plan'])) {
    die('<br /><br />
        <div class="ui-state-error center pad20 enorme">
            ...
            <a href="index.php?go=calendrier">
                <button class="bouton">
                    <span class="inline ui-icon ui-icon-arrowthickstop-1-w"></span>Retour calendrier
                </button>
            </a>
        </div>');
}
...
foreach ($listeTekos as $k => $v) {
    if (!in_array($v['categorie'], $catTekos)) {
        $catTekos[] = $v['categorie'];
    }
}
foreach ($listeMatos as $k => $v) {
    if (!in_array($v['categorie'], $catMatos)) {
        $catMatos[] = $v['categorie'];
    }
}
...
<div class="miniSousMenu gros" style="padding: 5px 10px 10px 10px;">
    <div class="inline quart leftText">
        <button class="bouton" onclick="retourCalendar()">
            <span class="inline ui-icon ui-icon-arrowthickstop-1-w"></span>Retour calendrier
        </button>
    </div>
    <div class="inline demi center">
        <div class="inline ui-state-highlight ui-corner-all pad5 gros"id="rappelPlanInfos">
            Modification de <i><b><?= $titrePlanMod; ?></b></i>	à <b>
            <?= $lieuPlanMod; ?></b>, pour <b><?php echo $benefPlanMod; ?></b>
        </div>
    </div>
    <div class="inline ui-state-focus ui-corner-all pad5">créateur : <?= $userPlanMod; ?></div>
...
<div class="gros">
    <div class="ui-state-default ui-corner-all pad10 leftText shadowOut">
        ...
        <div class="inline mid marge30r">
            <div class="inline" style="width:100px;">Titre :</div>
            <div class="inline"><input type="text" id="modPlanTitre" value="<?= $titrePlanMod; ?>" /></div>
            <br />
            <div class="inline" style="width:100px;">Bénéficiaire :</div>
            <div class="inline"><input type="text" id="modPlanBenef" value="<?= $benefPlanMod; ?>" /></div>
            <br />
            <div class="inline" style="width:100px;">Lieu :</div>
            <div class="inline"><input type="text" id="modPlanLieu" value="<?= $lieuPlanMod; ?>" /></div>
        </div>
        <div class="inline mid center enorme marge30l">
            DU : <input type="text" class="inputCal" id="modPlanStart" value="<?= $js.'/'.$ms.'/'.$as; ?>" size="12" />
            AU : <input type="text" class="inputCal" id="modPlanEnd" value="<?= $je.'/'.$me.'/'.$ae; ?>" size="12" />
        </div>
    ...
    <div class="ui-state-default ui-corner-all pad10 leftText shadowOut margeTop10">
        ...
        <br />
        <div class="center">
            <button class="bouton ui-state-highlight" id="modPlan_modMatosListe">MODIFIER LA LISTE</button>
        </div>
        <br />
        <?php

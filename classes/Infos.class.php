...
class Infos implements Iterator
{
    ...
    {
        $this->bddCx = null;
        $this->bddCx = new PDO(DSN, USER, PASS, [PDO::ATTR_PERSISTENT => true]);
    ...
    public function loadInfos($filtre, $filtre_key)
    {
        ...
        if ($nbResults == 0) {
            throw new Exception(Infos::NO_INFO." pour $filtre = $filtre_key dans ".$this->table);
        } elseif ($nbResults == 1) {
    ...
    // oblige à updater un champ lors de la sauvegarde au lieu de creer un enregistrement.
    // @OBSOLTETE : pas utilisé, mais conservé au cas ou
    public function forceLoaded($etat)
    {
        $this->loaded = $etat;
    }
    ...
    {
        if ($champ == Infos::ALL_DATAS) {
            return $this->datas;
        }
        if (!isset($this->datas[$champ])) {
            return false;
        }
    ...
    {
        $this->initPDO();
        // sauvegarde, si pas d'argument on va chercher l'id,
        // si pas d'id dans les data, on met 0 pour éviter les 'Notices' (pas grave mais propre)
        if ($filter == 'this') {
            if (isset($this->datas[$filterKey])) {
                $filter = $this->datas[$filterKey];
            } else {
                $filter = 0;
            }
        }
    ...
    private function updateBDD()
    {
        ...
        if ($q->rowCount() >= 1) {
            $colums = $q->fetchAll();
            // Si nouvelle clé ds tableau, on ajoute un champ
            foreach ($this->datas as $k => $v) {
                $exist = false;
    ...
    public function update($row, $val)
    {
        if ($this->rowExist($row) == true) {
            if ($this->updateChamp($row, $val)) {
                return Infos::UPDATE_OK;
            } else {
                throw new Exception(Infos::UPDATE_ERROR . ' ::updateChamp() IMPOSSIBLE !');
            }
        } else {
            if ($this->addChamp($row, $val) == true) {
                ...
                throw new Exception(Infos::UPDATE_ERROR . ' ::addChamp() IMPOSSIBLE !');
            }
        }
    }
    ...
    // Check si un champ est un index unique en BDD (CAD si le champ peut avoir
    ...
    private function addChamp($row, $val)
    {
        ...
        $char = '';
        // check du type de valeur du champ à ajouter
        if (is_numeric($val)) {
            ...
            if (ctype_digit($val)) {
                ...
                $typeRow = 'FLOAT('.$tailleChamp.' )'; // Si c'est un nombre à virgule
            }
            ...
            if (strlen($val) <= 64) {
                ...
                $typeRow = 'TEXT'; // Si c'est une grande chaîne
            }
        }
    ...
    // (fonction statique, peut être appellée sans créer d'instance de Infos)
    public static function removeChamp($table = '', $row = '')
    {
        ...
        $pdoTmp = new PDO(DSN, USER, PASS, [PDO::ATTR_PERSISTENT => false]);
    ...
    {
        return (current($this->datas) === false);
    }
}

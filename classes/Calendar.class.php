...
class Calendar implements Iterator, arrayaccess
{
    ...
    public function InitPlans($cS = -1, $cE = -1, $excludePlanId = null)
    {
        ...
        if ($this->calendarStart != -1 && $this->calendarEnd != -1) {
            $this->index_object->setFiltreSQL(
                "((`".Calendar::CAL_cSTART."` >= $this->calendarStart AND `".Calendar::CAL_cEND."` <= $this->calendarEnd)
    ...
    private function createTekosBusy($sousPlanId, $tekosList, $stamp = '', $titrePlan = '', $confirm = 0)
    {
        ...
        foreach ($tekosList as $tID) {
            $this->TekosBusy[$tID][] = [
                Plan::PLAN_cDETAILS_ID      => $sousPlanId,
    ...
    {
        if (!isset($this->Plans[$index]) || empty($this->Plans[$index])) {
            return;
        }
    ...
    // Initie les packs de matériel
    public function initPacks($arrayPacks = null)
    {
        if (is_array($arrayPacks)) {
    ...
    {
        return isset($this->Plans[$offset]);
    }
    ...
    {
        return isset($this->Plans[$offset]) ? $this->Plans[$offset] : null;
    }
    public function offsetSet($offset, $value)
    {
        if (!get_class($value) == 'Plan') {
            return false;
        }
        if (is_null($offset)) {
            $this->Plans[] = $value ;

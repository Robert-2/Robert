...
if ($action == 'add') {
    ...
    if ($addTekos->save()) {
        ...
        echo 'Impossible d\'ajouter le technicien...';
    }
}
...
if ($action == 'modif') {
    ...
    if ($modTekos->save()) {
        ...
        echo 'Impossible de modifier le technicien...';
    }
}
...
if ($action == 'delete') {
    try {
        ...
        if ($delTekos->deleteTekos() > 0) {
            ...
            echo "Impossible de supprimer le technicien...";
        }
    } catch (Exception $e) {
        echo $e->getMessage();
    }
...
if ($action == 'delTekFile') {
    ...
    if (! @unlink($filename)) {
        $result['Error'] = 'Erreur lors de la suppression de '.$file;
    } else {
        $result[] = $file;
    }
...
if ($action == 'fileList') {
    ...
    if (strpos($dirName, '..') !== false || strpos($dirName, '/') !== false) {
        $result['Error'] = 'Action non autorisée' ;
    }

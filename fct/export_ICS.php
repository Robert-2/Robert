...
function ICS_exporter($fichierICS = '../calendars/Robert.ics', $calName = 'Robert') {
...
// fin du fichier final     @TODO: vérifier si le fait d'ajouter un UID améliore la non-mise en cache du ICS par Google !
    ...
    // formatage SQL > PHP et suppression des accents (non supportés par Google)
    $search  = ['/"/', '/,/', '/\n/', '/\r/', '/:/', '/;/', '/\\//', '/é/', '/è/', '/ê/', '/à/', '/â/', '/î/', '/ô/', '/ç/'];
    $replace = ['\"', '\\,', '\\n', '', '\:', '\\;', '\\\\', 'e', 'e', 'e', 'a', 'a', 'i', 'o', 'c'];
    ...
    return file_put_contents($fichierICS, $output);
}

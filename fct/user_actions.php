...
if ($action == "select" || $action == 'load') {
    global $tmpUser ;
    createTmpUser($id);
    echo json_encode($tmpUser->getUserInfos());
    unset($selUser) ;
}
...
if ($action == "modifTheme") {
    ...
    if (($_SESSION["user"]->getUserInfos('id') != $_POST['id'])) {
        ...
        try {
            ...
            echo 'Impossible de modifier le thème...<br />'.$e->getMessage();
        }
    }
}
...
if ($action == "delete") {
    if ($_SESSION["user"]->isAdmin() !== true) {
        die("Vous n'êtes pas habilité à supprimer des utilisateurs !"); 
    }
    try {
...
if ($action == "supprInfoFromBDD") {
    ...
    if ($info == 'id' && $info == 'email'
            && $info == 'password' && $info == 'nom'
            && $info == 'prenom' && $info == 'level'
            && $info == 'theme' && $info == 'date_inscription'
            && $info == 'date_last_action'
            && $info == 'date_last_connexion' && $info == 'idTekos') {
    ...
    if (Infos::removeChamp(TABLE_USERS, $info)) {
        ...
        $retour['error'] = "Impossible de supprimer le champs $info de la base de donnes !";
    }

...
$(function() {
    ...
    $('#add_Matos_Rapide').click(function(){
        $('#addMatosModal').dialog({
            ...
            modal: true,
            buttons: {
                'Ok'      : function() { if ( ! addMatosToBDD () ) return ; $(this).dialog('close'); },
...
function displayTekosMatos(data) {
    if (!data.tekos || data.tekos.length < 1) {
        alert('La liste des techniciens est vide.\n\nAjoutez des techniciens pour pouvoir continuer...');
        window.location.href = "index.php?go=gens";
    }
    if (!data.matos || data.matos.length < 1) {
        alert('La liste du matériel est vide.\n\nAjoutez au moins un matériel pour pouvoir continuer...');
        window.location.href = "index.php?go=materiel";
    }
